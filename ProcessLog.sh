...
#如果第一次检查存在CPU或存在内存超过50%,则等待5s，否则退出
...
#若存在占用内存超过50%的进程，再次进行检查
if [[ $memsum -gt 0 ]];then
    for i in ${mem[*]};do
        eval $(ps -aux -q $i -h | awk -v memcout=0 '{if($4>=50) {memcout+=1;printf("mem2["memcount"]=%d;mem1["memcount"]=%.40s;mem3["memcount"]=%s;mem4["memcount"]=%2.2f;mem5["memcount"]=%2.2f;",$2,$11,$1,$3,$4);}} END{printf("memcount=%d",memcount);}')
    done
fi 

#若再次检查cpu还超过50%，则输出该进程的相关信息
