...
#内核版本
Kerv=`cat /proc/version | cut -d ' ' -f 3`

#运行时间和负载
Ostime=(`uptime | tr -s -c 'a-zA-Z0-9.:\' ' ' | cut -d ' ' -f 3-4,6 | tr -s -c 'a-z0-9.\n' ' '`)
...
#输出
echo "$time $hostname $Osv $Kerv ${Ostime[0]}_${Ostime[1]})_day,_${Ostime[2]}_hours,_${Ostime[3]}_minutes $fz ${cp[1]}% $total ${bi}% $wd ${note[0]} ${note[1]} ${note[2]}"
